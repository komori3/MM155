...
namespace SimpleDFS {
    ...
    struct State {
        ...
        State(const Input& input, Timer& timer_, double duration_ms_) : timer(timer_), duration_ms(duration_ms_) {
            N = input.N;
            for (int y = 0; y < 32; y++) {
                for (int x = 0; x < 32; x++) {
    ...
    std::pair<int, std::vector<std::string>> solve(const Input& input, double duration_ms) {
...
namespace ReversedDFS {
    ...
    struct State {
        ...
        // (y, x) ‚ÉŒü‚©‚Á‚ÄL‚Ñ‚Ä‚¢‚é–îˆó‚ª‘¶Ý‚·‚éƒZƒ‹‚ÌƒŠƒXƒg
            ...
            for (int y = 1; y <= N; y++) {
                for (int x = 1; x <= N; x++) {
                    ...
                    std::sort(cands[y][x].begin(), cands[y][x].end(), [&](const std::pair<int, int>& a, const std::pair<int, int>& b) {
                        ...
                        return ma == mb ? da < db : ma > mb; // mult ‚Å‚©‚¢‡
                    });
                }
            }
        }
...
int main(int argc, char** argv) {
    ...
    if (LOCAL_MODE) {
        ...
        out << best_moves.size() << '\n';
        for (std::string m : best_moves) out << m << '\n';
    }
    else {
